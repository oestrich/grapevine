defmodule Metrics.TelnetInstrumenter do
  ...
  def setup() do
    ...
    events =
      Enum.map(events, fn event ->
        ...
        Counter.declare(
          name: String.to_atom("grapevine_telnet_mssp_#{name}_count"),
          help: "Total count of tracking for telnet MSSP event #{name}"
        )

        [:grapevine, :telnet, :mssp | event]
      end)

    :telemetry.attach_many("grapevine-telnet", events, &handle_event/4, nil)
  end

  def handle_event([:grapevine, :telnet, :mssp, :start], _count, _metadata, _config) do
    Logger.debug("Starting MSSP", type: :mssp)
    Counter.inc(name: :grapevine_telnet_mssp_start_count)
  end

  def handle_event([:grapevine, :telnet, :mssp, :connected], _count, _metadata, _config) do
    Logger.debug("Connected to game", type: :mssp)
    Counter.inc(name: :grapevine_telnet_mssp_connected_count)
  end
  ...
  def handle_event([:grapevine, :telnet, :mssp, :option, :sent], _count, _metadata, _config) do
    Logger.debug("Sending telnet option", type: :mssp)
    Counter.inc(name: :grapevine_telnet_mssp_option_sent_count)
  end
  ...
  def handle_event([:grapevine, :telnet, :mssp, :line_mode, :sent], _count, _metadata, _config) do
    Logger.debug("Sending line mode", type: :mssp)
    Counter.inc(name: :grapevine_telnet_mssp_line_mode_sent_count)
  end

  def handle_event([:grapevine, :telnet, :mssp, :term_type, :sent], _count, _metadata, _config) do
    Logger.debug("Sending term type", type: :mssp)
    Counter.inc(name: :grapevine_telnet_mssp_term_type_sent_count)
  end
