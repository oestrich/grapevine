defmodule Metrics.GameEventInstrumenter do
  ...
  def handle_event([:grapevine, :game_events, :create, status], count, metadata, _config) do
    Logger.debug(fn ->
      "Creating #{count} new game event(s) for game #{metadata[:game_id]} was a #{status}"
    end)
  end

  def handle_event([:grapevine, :game_events, :update, status], count, metadata, _config) do
    Logger.debug(fn ->
      "Updating #{count} game event(s) for game #{metadata[:game_id]} was a #{status}"
    end)
  end

  def handle_event([:grapevine, :game_events, :delete, status], count, metadata, _config) do
    Logger.debug(fn ->
